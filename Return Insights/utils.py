...
class Calculator:
    ...
    def sharpe_ratio(self, returns_list, risk_free_rate):
        # Check for empty list to avoid division by zero
        if len(returns_list) == 0:
            return 0  
        ...
        std_dev = self.standard_deviation(returns_list)
...
class Capm:
    ...
    def regression(self, returns_asset, returns_market, risk_free_rate):
        ...
        return {
            ...
            "p_alpha": alpha, # The p-value for alpha (indicates the significance of alpha in the regression).
